...
// Fetch bulk vote packages for the current organizer
$bulkVotes = [];
try {
    $stmt = $pdo->prepare("
        SELECT 
        ...
        FROM bulk_vote_packages bv
        LEFT JOIN events e ON bv.event_id = e.id
        WHERE bv.organizer_id = ? OR e.organizer_id = ?
        ORDER BY bv.created_at DESC
    ");
    $stmt->execute([$user['id'], $user['id']]);
    $bulkVotes = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>Bulk Votes - E-Cast Voting</title>
        ...
        <main class="main-content">
            ...
            <header class="header">
                <div class="header-left">
                    ...
                    <div>
                        <h1 class="page-title mb-0">Bulk Votes</h1>
                    ...
                    <div class="card-body" style="padding: 0;">
                                        ...
                                        <tr>
                                            <td colspan="5" class="text-center py-4">
                                                <div class="empty-state">
                                                    ...
                                                    <h5 class="text-muted">No Bulk Vote Packages Found</h5>
                                                    <p class="text-muted">Bulk vote packages will appear here once you create voting packages for your events.</p>
                                                    <button class="btn btn-primary" onclick="openAddBulkVotesModal()">
                                                        <i class="fas fa-plus"></i> Add Bulk Votes Package
                                                    </button>
                                                </div>
                                            </td>
                                        </tr>
                                    <?php else: ?>
                                        <?php foreach ($bulkVotes as $index => $bulkVote): ?>
                                            <tr>
                                                <td><?= str_pad($bulkVote['id'], 5, '0', STR_PAD_LEFT) ?></td>
                                                <td><?= number_format($bulkVote['vote_count'] ?? 0) ?></td>
                                                <td><?= number_format($bulkVote['package_price'] ?? 0, 2) ?></td>
                                                <td><?= date('Y-m-d', strtotime($bulkVote['created_at'] ?? 'now')) ?></td>
                                                <td>
                                                    <div class="d-flex gap-1">
                                                        <button class="btn btn-success btn-sm" onclick="editBulkVote(<?= $bulkVote['id'] ?>)" title="Edit Package">
                                                            <i class="fas fa-edit"></i>
                                                        </button>
                                                        <button class="btn btn-danger btn-sm" onclick="deleteBulkVote(<?= $bulkVote['id'] ?>)" title="Delete Package">
                        ...
                        <div style="padding: 1.5rem;">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <?php if (!empty($bulkVotes)): ?>
                                        <small class="text-muted">Showing <?= count($bulkVotes) ?> of <?= count($bulkVotes) ?> entries</small>
                                    <?php else: ?>
                                        <small class="text-muted">No entries to display</small>
                                    <?php endif; ?>
                                </div>
                                <?php if (!empty($bulkVotes) && count($bulkVotes) > 10): ?>
                                <nav>
