...
// Fetch registrations for the current organizer's events
$registrations = [];
try {
    $stmt = $pdo->prepare("
        SELECT 
        ...
        LEFT JOIN nominees n ON r.nominee_id = n.id
...
<head>
    ...
    <title>Registration List - E-Cast Voting</title>
...
<body>
        ...
        <main class="main-content">
            ...
            <header class="header">
                <div class="header-left">
                    ...
                    <div>
                        <h1 class="page-title mb-0">Registration List</h1>
                    ...
                    <div class="card-body" style="padding: 0;">
                                        ...
                                        <tr>
                                            <td colspan="12" class="text-center py-4">
                                                <div class="empty-state">
                                                    ...
                                                    <p class="text-muted">Event registrations will appear here once users register for your events.</p>
                                                    <a href="../voter/events.php" class="btn btn-primary" target="_blank">
                                                ...
                                                <td>
                                                    <div style="width: 40px; height: 40px; background: #f8f9fa; border: 1px solid var(--border-color); border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                                                        <?php if (!empty($registration['profile_image'])): ?>
                                                            <img src="<?= htmlspecialchars($registration['profile_image']) ?>" alt="Profile" style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover;">
                                                        <?php else: ?>
                                                            <i class="fas fa-user" style="color: var(--text-muted);"></i>
                                                        <?php endif; ?>
                                                    </div>
                                                </td>
                                                <td><?= htmlspecialchars($registration['event_title'] ?? 'N/A') ?></td>
                                                <td><?= htmlspecialchars($registration['category_name'] ?? 'N/A') ?></td>
                                                <td><?= htmlspecialchars($registration['nominee_name'] ?? 'N/A') ?></td>
                                                <td><?= htmlspecialchars($registration['phone_number'] ?? 'N/A') ?></td>
                                                <td><?= htmlspecialchars($registration['nominee_code'] ?? 'N/A') ?></td>
                                                <td><?= htmlspecialchars($registration['custom1'] ?? '') ?></td>
                                                <td><?= htmlspecialchars($registration['custom2'] ?? '') ?></td>
                                                <td><?= htmlspecialchars($registration['custom3'] ?? '') ?></td>
                                                <td><?= htmlspecialchars($registration['custom4'] ?? '') ?></td>
                                                <td><?= htmlspecialchars($registration['custom5'] ?? '') ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </tbody>
                        ...
                        <div style="padding: 1.5rem;">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <?php if (!empty($registrations)): ?>
                                        <small class="text-muted">Showing <?= count($registrations) ?> of <?= count($registrations) ?> entries</small>
                                    <?php else: ?>
                                        <small class="text-muted">No entries to display</small>
                                    <?php endif; ?>
                                </div>
                                <?php if (!empty($registrations) && count($registrations) > 10): ?>
                                <nav>
    ...
    <script>
        // Additional functionality for registration page
